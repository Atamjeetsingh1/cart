...
function About() {
  const [btn,setBtn]=useState('Enable Dark Mode');
    const [myStyle,setmyStyle]=useState({ color:'black',
        backgroundColor:'white'
          ...
          //Dark Mode enable kr rhe h
        function toggleStyle()
        {
         if(myStyle.color==='black')//strictly equalsto
     ...
     {setmyStyle(
      ...
      setBtn('Enable Dark Mode')
        ...
        //return se phle likhenge hmessha
  return (
    <> 
  ...
  <button onClick={toggleStyle} value={btn} type="button" className="btn btn-primary">{btn}</button>
  </div>
</div></div>
    </>
  )
